/* ============================================================
   ...
   相机站在球心,34 个视觉作品位置贴满内球面,广角镜头给球感。
...
import { createStarfieldDome } from "./starfield-dome.js";
...
const CONFIG = {
  ...
  maxTex: 256,           // 贴图长边(控显存)
  seed: 1337,
  brand: "STARRY ARCHIVE",
  cats: {                // 分类(键 = 作品 id 首字母)
    a: { en: "VISUAL", zh: "视觉卷" },
  },
  counts: { a: 34 },         // 34 张已收录作品，无空位
};
...
const sky = createStarfieldDome({ compact: innerWidth < 768 });
scene.add(sky);
...
scene.add(camera);
// Compile the procedural sky before starting the short entrance choreography.
...
function entranceOffsets() {
  const e = THREE.MathUtils.clamp(entrance.p, 0, 1);
  return { yaw: 0.18 * (1 - e), pitch: -0.06 * (1 - e), fov: 30 * (1 - e), settled: e >= 0.999 };
}
let lastFov = 0;
...
function build(){
  ...
  // Start the spatial entrance immediately instead of waiting on a decorative counter.
...
function placeSet(set, mode){
  ...
  set.forEach((t, i) => {
    ...
    if (mode === "fly") {
      const from = t.basePos.clone().multiplyScalar(1.7);
      m.position.copy(from); m.scale.set(t.baseScale.x, t.baseScale.y, 1); m.material.opacity = 0;
      const delay = 0.35 + (i / Math.max(1, set.length - 1)) * 0.15;
      m.lookAt(0,0,0);
      gsap.to(m.position, { x: t.basePos.x, y: t.basePos.y, z: t.basePos.z, duration: 0.9, delay, ease: "power2.out", onUpdate: () => m.lookAt(0,0,0) });
      gsap.to(m.material, { opacity: 1, duration: 0.55, delay, ease: "power1.inOut" });
    } else if (mode === "pack") {
...
function startArchive(){
  if (started) return;
  started = true;
  placeSet(visibleSet(), reducedMotion ? "none" : "fly");
  // Lens push first, then the photographs approach and settle on the dome.
  if (reducedMotion) { entrance.p = 1; veil.style.opacity = "0"; veil.style.visibility = "hidden"; hint.classList.add("show"); }
  else {
    gsap.fromTo(orb, { scale: 0.65 }, { scale: 1, duration: 0.65, ease: "power2.out" });
    gsap.to(entrance, { p: 1, duration: 1.4, ease: "power2.inOut" });
    gsap.to(veil, { opacity: 0, duration: 0.3, ease: "power1.out", onComplete: () => { veil.style.visibility = "hidden"; } });
...
function tick(){
  ...
  if (started && !detailOpen) {
    for (const tile of tiles) {
      // Delayed entrance tweens are not yet "tweening": do not let idle bobbing
      // snap the distant starting positions back to the dome before they begin.
      if (!entranceOffsets().settled || !tile.onDome || gsap.isTweening(tile.mesh.position)) continue;
...
/* ---------- 调试 API ---------- */
window.__app = {
  view, tiles, entrance, CONFIG, camera, sky,
